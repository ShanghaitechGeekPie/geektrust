...
```
┌────────────┐  SOCKS5/HTTP   ┌──────────────────────┐  TLS(TCP流/L3帧)      ┌──────────────────┐
│  应用程序    │ ◄────────────► │  geekTrust           │ ◄════════════════════► │  aTrust 代理网关    │
│ (curl/浏览器)│   本地代理入口   │  (本项目,用户态实现)   │   TCP stream / L3    │  <gateway>:441    │
└────────────┘                │                      │                       └────────┬─────────┘
                              │  ┌────────────────┐  │  HTTPS/JSON(控制面)           │
                              │  │ 登录/配置模块    │──┼──────────────────────────────► │
                              │  └────────────────┘  │                                ▼
...
- **不创建系统虚拟网卡、不修改路由表、不需要 root**。所有流量从本地 SOCKS5/HTTP 代理入口进入。
  TCP 优先使用网关的流式通道;UDP 和流式通道不可用时的 TCP 使用 L3 每连接认证,
  以 gVisor 用户态 IPv4 endpoint 重建后封装为 IPv4 包转发。
...
- VIP 帧 `addrType`:`1`→IPv4(4 字节)+元数据(2 字节);`4`→IPv6(16 字节)+
  元数据(2 字节);`5`→IPv4(4 字节)+IPv6(16 字节)+元数据(2 字节),共 22 字节。
- `addrType=1/5` 时 IPv4 VIP 取 `addr[0:4]`(如 `10.19.240.43`);
  `addrType=4` 时 IPv6 VIP 取 `addr[0:16]`,`addrType=5` 时取 `addr[4:20]`。
  VIP 仅用作数据包的源地址(SNAT 标记),不创建网卡。

> 上海科大网关线上实测分配 `addrType=1` IPv4 VIP,即使 441 接入线路本身使用 IPv6
> 也是如此;因此当前数据面不能构造有合法源地址的 IPv6 目标连接。
...
TCP 不需要经过 §6 至 §8 的 L3 小包数据面。客户端对目标应用所属 node group 的
`<gateway>:441` 建立 TLS,在**同一次写入**中发送:

```
05 01 81 | 53 03 <BE16 JSON长度> <TCP auth JSON> | 05 01 01 <ATYP> <目标> <BE16端口>
```

认证 JSON 包含 `sid`、`appId`、`deviceId`、`connectionId`、`userName`、
进程环境和目标地址。浏览器登录路径没有桌面客户端 signKey;与 L3 每连接认证相同,
实测网关接受空字符串 `xRequestSig`。

网关依次返回 `05 81`、`53 00 <BE16长度> <OK/JSON>` 和 SOCKS 风格连接结果。
线上网关的 RSV 字节可能非零,客户端只校验版本、状态和地址类型,并完整读走
绑定地址及端口。建立后应用字节使用以下帧:
...
认证与目标地址必须合并写入,也不能在连接结果前额外发送空数据帧;否则部分网关
会接受初始化后关闭第一段应用数据。流式通道不可用或直连命令不兼容时,
回退到下述 L3 路径。
...
L3 路径中的每条 UDP 流和兼容回退 TCP 流,先做一次每连接认证以获取
`connectToken`。正常 TCP 使用 §5.5 流式通道。
...
本节描述 L3 数据面,供 UDP 和兼容回退 TCP 使用。
...
3. 按 §4.2 选择目标 IP、TCP `appId` 和可选域名;优先建立 §5.5 流式通道,
   流式通道不可用或网关不兼容时回退到 §6 至 §8 L3 路径;无法解析时回
...
| TCP RST / `connection refused` | 目标主机明确拒绝该端口,不是网关 churn | 立即返回,不做相同目标的内部重试 |
...
- 握手超时可退避重试(最多 4 次,间隔递增);每连接认证 `10000008 auth in progress`
  用较短退避重试。TCP RST/connection refused 与持久认证拒绝不重试。
- TCP 选线的成功 TLS socket 直接进入认证,不执行“探测后关闭、再连接一次”。
  UDP/L3 回退复用已建立的隧道承载多条 flow。
...
- 首选线路先发起 TLS,其余线路短间隔错峰竞速;首个成功连接立即返回并成为下次首选,
  不等待慢线路。失败线路进入冷却;隧道错误码 `10000002~4`/`99700001` 触发换线。
